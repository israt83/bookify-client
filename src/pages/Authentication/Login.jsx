...
//       toast.error(prev => ({ ...prev, password: "Incorrect password. Please try again." }));
//     } else if (err?.code === 'auth/user-not-found') {
//       toast.error(prev => ({ ...prev, email: "No user found with this email." }));
//     } else {
//       toast.error(err?.message);
//     }
//   }
...
//         <div
//           className="hidden bg-cover bg-center lg:block w-[450px] h-[440px]"
//           style={{
...
//             <img className="w-44 h-28" src={logo2} alt="" />
//           </div>
...
//               className="flex cursor-pointer items-center justify-center  text-gray-600 transition-colors duration-300 transform  rounded-lg   hover:bg-gray-50 "
//             >
...
const Login = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { signIn, signInWithGoogle, githubLogin, user, loading } = useContext(AuthContext);
  const [showPassword, setShowPassword] = useState(false);
